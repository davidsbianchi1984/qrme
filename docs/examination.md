...
<tr>
<td valign="top">15</td>
<td valign="top">A record set large enough to be exhaustive is necessarily coarse: detail written per record does not scale to tens of thousands of them, so every record answers with what its whole category shares and a reader learns nothing the category name would not have told them.</td>
<td valign="top">The coarse half and the specific half are <strong>obtained by different means and merged specific-first</strong>. The carried table answers for the family and answers offline; a per-record narrative is fetched at the point of use and <strong>parsed into the same structured fields the record stores</strong> (<code>company.<wbr>role_<wbr>specifics</code>) — short noun phrases, deduplicated case-blind against the shared half, capped, and constrained to information handling rather than acts. The count of specific entries is <strong>returned with them</strong>, so a surface can distinguish a category-only answer from one that was never fetched.</td>
<td valign="top">516 of 45,153 records carry detail of their own; the other 44,637 now answer with their own specifics ahead of their family's. Every failure path — nothing reachable, a refusal, an unparseable answer, an empty narrative — falls back to the shared half, which is the behaviour that existed before, so the screen degrades to correct rather than to empty.</td>
<td valign="top"><code>qrme/<wbr>company.py</code>,<br><code>qrme/<wbr>occupations.py</code> — <code>test_<wbr>the_<wbr>study_<wbr>says_<wbr>what_<wbr>this_<wbr>job_<wbr>needs.py</code></td>
</tr>
<tr>
<td valign="top">16</td>
<td valign="top">Equipping a newly created identity with third-party capability requires a credential the platform does not hold and must not mint: either the operator is sent elsewhere to do it by hand, or the platform grants access it has no right to grant.</td>
<td valign="top">The capability is created in a <strong>deliberately unauthorized state</strong>. A narrative naming the tools of a trade is matched against a connector catalogue by <strong>counted word overlap</strong> (<code>company.<wbr>match_<wbr>tools</code>), matches become a selectable step, and the creating act writes a connector row with <code>authorized_<wbr>at</code> NULL for every connector whose <code>needs_<wbr>first</code> is not <code>nothing</code>. The credential arrives later through its own door. Tools with no connector are <strong>returned by name</strong> rather than dropped, so the answer is about the job rather than about the catalogue.</td>
<td valign="top">A new identity is handed a door it cannot open: the row exists, is owner-gated and revocable, and every capability invocation through it is refused until the credential is supplied separately. The operator never leaves the creating screen, and the platform never holds a credential it was not given.</td>
<td valign="top"><code>qrme/<wbr>company.py</code>,<br><code>qrme/<wbr>routers/<wbr>apps.py</code>,<br><code>qrme/<wbr>catalog.py</code> — <code>test_<wbr>the_<wbr>study_<wbr>says_<wbr>what_<wbr>this_<wbr>job_<wbr>needs.py</code>,<br><code>test_<wbr>the_<wbr>new_<wbr>hire_<wbr>is_<wbr>kitted_<wbr>out_<wbr>in_<wbr>the_<wbr>seat.py</code></td>
</tr>
<tr>
<td valign="top">17</td>
<td valign="top">An exhaustive record set answers from a handful of category blocks, so a record's leading detail is its category's and is wrong wherever the category is broad: a housekeeper filed under hospitality leads with till reconciliation.</td>
...
Eleven rows from the carried table, as the reader returns them. *Family
...
leading with the wrong half. Three rows were written by hand: their own
line leads and the group follows it, and the family block no longer
fills the line behind them — `Housekeeper`, the row in screen 220, read
*room status reporting · linen control · order taking · stock rotation*
until it did not. One row has no group at all, and is listed as the
honest gap it is.
...
| Housekeeper | Hospitality, food & retail | Cleaning and housekeeping (written; own line leads) | order taking / stock rotation / hygiene record keeping | room status reporting / linen control / room and area checklists |
| Train Conductor | Transport & logistics | Transport operations (written; own line leads) | route planning / manifest handling / hours and rest recording | ticket inspection / dispatch procedure / timetable and roster following |
| Legal Secretary | Law & public administration | Secretarial and executive support (written; own line leads) | statute reading / precedent research / document drafting | diary management / document formatting / diary and travel arranging |
