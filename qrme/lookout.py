...
def plant(profile_id: str, url: str, every_hours: float, pdi=None) -> dict:
    ...
    try:
        # fetch.render for a page; fetch.listen when the URL is itself a
        # recording — the vault's ears seal the words said in it, under the
        # same capture key and change-memory, so the read-back, the
        # changed_at, the chat prompt and the letter all read a transcript
        # exactly the way they read a page. A deployment without ears fails
        # the cycle in words and the `trouble` line carries the reason;
        # there is no silent stand-in for hearing.
