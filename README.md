...
| Release | What landed |
|---|---|
| **3.0.0** | **Every avenue functions properly inside the apps** — the celebration release the ROADMAP reserved. The gate was never a checklist: a person who has never seen this code picks any road on the map — chat, voice, avatar, video, AR, VR, the watch, a profile acting through its own connections — and drives it to the end without finding a wall. `tools/walkthrough.py` drives all of them over real doors: thirty steps, zero walls, and six photographs of the driven console in `docs/walkthrough/`. Store distribution ships as a funded follow-up, by the owner's recorded decision — the tag waited on the roads, not on a fee. |
