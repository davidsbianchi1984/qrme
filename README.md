...
| Dials | A profile's / robot's **disposition** — throttle & behavior sliders (`qrme/dials.py`), part of who it is, not a remote control: each dial is 0–100 (50 = as written). **System** — `pace` (the throttle: unhurried ⟷ eager), `autonomy`, `verbosity`; **behavior** — `warmth`, `formality`, `humor`, `assertiveness`; **intimacy** — an 18+-only dial, present and effective only on an adult-mode profile (hard-clamped to 0 otherwise) and, even at full, raising flirtation/affection *within the persona's boundaries and strict moderation* — never explicit on demand. The dials ride on the persona system prompt (chat, compose, rooms, robot speech all inherit them) and a robot reads pace/autonomy/assertiveness as a motion behavior profile. `GET`/`PUT /profiles/{id}/dials` and `/robots/{id}/dials`, owner-only; the watch surfaces the live throttle. Dials shape style/pace/behavior only — never identity, boundaries, age-gating, or the command allowlist |
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="33%"><a href="docs/screens/66-dials.svg"><img src="docs/screens/66-dials.svg" width="210" alt="Dials"></a><br><sub><b>66</b> · Dials</sub></td>
