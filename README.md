...
**Current release: v3.4.0** ([changelog](CHANGELOG.md)) — one of three products
...
<table>
  <tr>
    ...
    <td align="center" width="25%"><a href="docs/screens/194-the-vastscape.svg"><img src="docs/screens/194-the-vastscape.svg" width="165" alt="A party you can find"></a><br><sub><b>194</b> · The vastscape<br>the wide view of everything running — drawn in the console's design, the starters' own portraits</sub></td>
  </tr>
</table>
...
<table>
  <tr>
    ...
    <td align="center" width="25%"><a href="docs/screens/32-moderation.svg"><img src="docs/screens/32-moderation.svg" width="165" alt="Moderation"></a><br><sub><b>32</b> · Moderation<br>review before a doubtful turn ships — drawn in the console's design</sub></td>
...
| Release | What landed |
|---|---|
| **3.4.0** | **The attention layers are the platform's own, and they train** — a persona network (`qrme/persona_net.py`), a two-layer, two-head transformer in numpy with the forward and backward passes written out and gradient-checked, runs over the person's last twelve turns before every reply: each turn's attention logit carries that turn's degree of engagement through a learned weight, the softmax temperature is set by the current engagement, and the readout — expected engagement next turn and four emphases — goes into the prompt with the attended turns quoted and into `persona_conditioning` as a row per reply. It rides the one prompt builder every speaking surface uses, so chat, rooms, letters, check-ins and hired seats are conditioned alike, and the Studio Agent is conditioned on its owner's turns. Fine-tuning fits the weights offline on the profile's own history; they rest as AES-GCM ciphertext under a per-deployment key bound to the profile, ride to the vault sealed, and are erased with the profile. `GET /profiles/{id}/persona-net` reports the state; mechanism 18 on the examination page |
| **3.3.1** | **The deploy block checks its own versions, and the catalogue is examined** — `docker/beta-versions.sh` is the last line of the runbook's deploy block: it reads the three public names from the same `.env` the compose line uses, prints what each answers beside the version in this checkout, and exits non-zero on a disagreement. The from-outside check stays for reachability, but it has two shells and a choice, and the choice has been made wrong three times running; this line has no choice in it. The examination page carries the group tier with the build's numbers and ends with ten positions shown three ways — family alone, now, and the group that made the difference — generated from the reader and held by a guard, with the two floors it stands on registered in the ratchet registry |
| **3.3.0** | **A position says what it does, not what field it is in** — 45,153 positions and 529 written by hand; the rest took their skills from one of sixteen family blocks, so a Commercial Housekeeper claimed till reconciliation and a Radiologist claimed triage by severity. A *group* is the tier between the title and the family: the shape of the work, matched the way families are, merged own then group then family. Sixty-four groups reach 29,710 positions (65.8%), a ratchet holds coverage from sliding back, and twenty-two guards hold the ordering decisions — teaching before its subject, engineering after the trade, a coach is also a vehicle, a hairspring is a watch part, a cat is a catalogue. Also held: the cloud gateway answers for both products or neither |
| **3.2.1** | **The release that could not build, and the guard that would have caught it** — 3.2.0 bumped `app/package-lock.json` by substituting the old version number for the new one, and the lockfile carries the project's own version alongside every dependency's. The project had been at 3.1.10; so had `ejs`, four levels down. There is no `ejs@3.2.0`, so `npm ci` answered 404 and every image on all three products died before a line compiled. `ejs` is back where it was, and a fingerprint of the lockfile with the project's own two version fields masked out now fails the suite if a release ever moves a dependency again. Also held: the cloud gateway must answer for both products or neither, which was a rule living in a comment |
| **3.2.0** | **Re-evaluate the job, and the rest of what the study found reaches the hire** — everything a study produced was written at the signature and nowhere else, so correcting a bad one meant retiring the employee and making a new person. Re-evaluate runs the same study on a filled seat and Keep carries it through to whoever is already in it. Connections land three ways rather than being dropped, a fifth rung offers the trade's tools as connectors that stay inert until you hand over the credential, and every synthetic professional stopped proposing work under "software". Also caught: a truncation marker was being filed as an employee's knowledge of its own profession |
| **3.1.10** | **The rest of what the study found reaches the hire** — skills were the only one of four lists that went anywhere. `hire` never read the connections at all: the people, departments and outside bodies a job has to reach were fetched, stored, and read by nobody. They land three ways now — filed as source material so replies ground on them, cross-matched against the roster, and kept on the profile as `works_with`. A fifth rung joins the ladder for the trade's tools, matched against the connectors, where ticking chooses and the signature creates a connector that stays unauthorised until you hand over the credential. And every synthetic professional stopped proposing work under "software": the sixteen occupation families map onto the sixteen exchange industries, so a hired radiologist gets `healthcare` |
| **3.1.9** | **The study says what *this* job needs** — 516 of 45,153 rows carry skills of their own and the other 44,637 inherit their family's, so a browse of the whole table read as sixteen jobs repeated. `company.role_specifics` reads the study that was just fetched and returns the two lists the seat stores: what it found about this job leads, what the pool knows about the family fills in behind, deduped case-blind. The card says which is which, because six generic skills look identical whether the study found nothing specific or was never reachable to be asked. Entries stay records and coordination rather than acts, and every failure path leaves the pool's answer standing |
| **3.1.8** | **Eyes, ears, hands and a body — without leaving the seat** — kitting out a new hire meant walking out of the hire: the screen lived in the employee file, the speaker in the Workshop, the robot on the settings shelf, the face in the studio. The Company Builder walks all four as a ladder now — each rung opens the next and the last one is the signature — and because a seat has no profile until it is signed, the choices are held and the hire fits them, reporting by name any piece that would not go on rather than letting one failed fitting look like a failed hire. Underneath it: a row's own skills lead its family's, so a browse of 45,153 positions stops reading as sixteen positions repeated; "Home Care Attendant" is out of hospitality and thirteen home-care roles are written out by hand; and the robot shelf, which the phone-width photograph caught at 6,974 pixels tall, is 958 |
| **3.1.7** | **Every job on earth, carried offline** — the Company Builder fell back to three canned seats when the model did not parse; it carries 45,147 positions now (516 written by family, 1,276 taxonomy occupations, 43,355 reported job titles) with the digital skills and connections each needs, searchable by what the job *does* rather than its name. `/occupations` opens the pool from the Builder, and **Download knowledge** makes the study a step the founder reads — skills and connections off the carried pool, the working knowledge fetched and kept on the seat, every line editable — before Sign and hire, which now waits for it |
...
| **3.0.2** | **The video road is owner-only, and the glyph fills the box** — the door that puts a profile on the video road, sets its daily spend ceiling, and picks its render provider took no token at all, so anyone holding a profile id (they ride on printed stickers) could spend that owner's budget; it is owner-gated now, on both the set and the read. And the reason the purple box stayed black is closed: pressing the video glyph on a seat you own puts that profile on the video road, so its next room turn renders through fal.ai — your own seats only, and a peek restores the road it was on so it never keeps spending. |
