...
| Release | What landed |
|---|---|
| **2.9.8** | **The screens render** — the stage offers "On your headset" wherever the browser can answer for one, and opens the same room as a WebXR session in the headset's own browser: VR stands the seats around you, AR hands compositing to the device's own passthrough. One shared ring module and one photo resolver keep the flat stage and the headset showing the same room under the same rules, guards hold the headset's source to the stage's no-capture promise, and a band pairs with every face a watch may hold. |
