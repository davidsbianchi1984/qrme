...
**Current release: v1.3.0** ([changelog](CHANGELOG.md)) — one of three products
...
| Release | What landed |
|---|---|
| **1.4.0** | **A slept tab does not go quietly deaf** — a backgrounded page has its speech recogniser ended by the browser with no error, and the console had no `visibilitychange` handler anywhere, so the orb's caption, the room's mic button and the line saying the room could hear you all stayed lit over a microphone that had stopped; the two relight loops (the agent's orb, the room's standing ear) stop restarting into a tab that cannot run one, both surfaces now read *this tab is in the background* in all ten languages and stand back up on return, and dictation — a press, not a decision — goes down with them and stays down |
