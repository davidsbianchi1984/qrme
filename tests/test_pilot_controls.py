"""Pilot controls: throttles and behavior sliders for profiles and robots.
Dials shape style/pace/behavior and ride on the persona prompt; intimacy is
...
from qrme import db, persona, pilot
...
def test_dials_default_to_neutral_and_say_nothing(client):
    pid = _profile(client)
    r = client.get(f"/profiles/{pid}/pilot").json()
    ...
    assert "Pilot settings" not in _prompt(pid)   # neutral = silent


def test_sliders_shape_the_persona_prompt(client):
    pid = _profile(client)
    client.put(f"/profiles/{pid}/pilot",
               json={"values": {"pace": 90, "humor": 15, "warmth": 80}})
    prompt = _prompt(pid)
    assert "Pilot settings" in prompt
    ...
    # Clamped to range.
    client.put(f"/profiles/{pid}/pilot", json={"values": {"pace": 999}})
    assert client.get(f"/profiles/{pid}/pilot").json()["values"]["pace"] == 100
...
def test_intimacy_is_18plus_only(client):
    ...
    assert "intimacy" not in [d["name"]
                              for d in client.get(f"/profiles/{pid}/pilot"
                                                  ).json()["dials"]]
    r = client.put(f"/profiles/{pid}/pilot",
    ...
    dials = {d["name"]: d for d in client.get(f"/profiles/{apid}/pilot"
                                              ).json()["dials"]}
    assert dials["intimacy"]["adult_only"] is True
    client.put(f"/profiles/{apid}/pilot", json={"values": {"intimacy": 90}})
    prompt = _prompt(apid)
...
def test_robot_dials_map_to_a_behavior_profile(client):
    ...
    r = client.get(f"/robots/{robot}/pilot").json()
    # A robot is piloted on pace/autonomy/behavior — never intimacy.
    assert all(d["group"] != "intimacy" for d in r["dials"])
    out = client.put(f"/robots/{robot}/pilot",
...
def test_pilot_is_owner_only(client, profile_id):
    r = client.put(f"/profiles/{profile_id}/pilot",
                   json={"values": {"pace": 90}},
