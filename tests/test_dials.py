...
from qrme import db, dials, persona
...
def test_dials_default_to_neutral_and_say_nothing(client):
    pid = _profile(client)
    r = client.get(f"/profiles/{pid}/dials").json()
    ...
    assert "How you're currently dialed in" not in _prompt(pid)   # neutral = silent


def test_sliders_shape_the_persona_prompt(client):
    pid = _profile(client)
    client.put(f"/profiles/{pid}/dials",
               json={"values": {"pace": 90, "humor": 15, "warmth": 80}})
    prompt = _prompt(pid)
    assert "How you're currently dialed in" in prompt
    ...
    # Clamped to range.
    client.put(f"/profiles/{pid}/dials", json={"values": {"pace": 999}})
    assert client.get(f"/profiles/{pid}/dials").json()["values"]["pace"] == 100
...
def test_intimacy_is_18plus_only(client):
    ...
    assert "intimacy" not in [d["name"]
                              for d in client.get(f"/profiles/{pid}/dials"
                                                  ).json()["dials"]]
    r = client.put(f"/profiles/{pid}/dials",
    ...
    idials = {d["name"]: d for d in client.get(f"/profiles/{apid}/dials"
                                               ).json()["dials"]}
    assert idials["intimacy"]["adult_only"] is True
    client.put(f"/profiles/{apid}/dials", json={"values": {"intimacy": 90}})
    prompt = _prompt(apid)
...
def test_dials_are_owner_only(client, profile_id):
    r = client.put(f"/profiles/{profile_id}/dials",
                   json={"values": {"pace": 90}},
